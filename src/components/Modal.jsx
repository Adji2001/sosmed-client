...
const Modal = () => {
  ...
  const submitHandler = async (e) => {
    ...
    dispatch(addPost(user._id, desc, file))
  }

  return (
    <>
      <button
        className='flex items-center justify-center h-10 w-10 text-slate-100 hover:bg-slate-600 rounded-full p-1 hover:cursor-pointer relative'
        type="button"
        title="Create post"
              ...
              <form onSubmit={submitHandler} className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="relative px-6 flex-auto">
                  <div className="my-2 text-slate-500 text-lg leading-relaxed flex flex-col gap-4">
                    <input onChange={(e) => setDesc(e.target.value)} type="text" placeholder={`What do you thing, ${user.username}?`} className="focus:border-b-2 focus:outline-none border-b-2" />
                    <label htmlFor="file" className="cursor-pointer">
                        {file ? (
                          <img src={URL.createObjectURL(file)} alt="web" className='bg-slate-100 mt-3 w-2/6 object-cover' />
                        ) : (
                          <>
